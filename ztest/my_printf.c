...
static int check_nan(float val, int *total) {
    if (isnan(val)) {
        *total += putnstr((const unsigned char*)"NaN", 3);
        ...
        if (val > 0) *total += putnstr((const unsigned char*)"+Inf", 4);
        else *total += putnstr((const unsigned char*)"-Inf", 4);
        return 1;
    }
    return 0;
}
...
static void float_to_str(float val, int precision, bool add_plus, unsigned char *buf) {
    int pos = 0;
    if (val < 0) {
        buf[pos++] = '-';
        val = -val;
...
static void float_to_exp_str(float val, int precision, bool add_plus, unsigned char *buf) {
    int pos = 0;
    if (val < 0) {
        buf[pos++] = '-';
        val = -val;
...
static int output_int(long val, int radix, bool add_plus, bool left_justify, int width, bool uppercase, unsigned char *buf) {
    char tmp[64];
    if (val < 0) {
        ltoa(val, tmp, radix);
    } else if (add_plus) {
    ...
    while (*fmt) {
        ...
        if (*fmt == 'l') {
            ...
            if (*fmt == 'd') {
                ...
                total += output_int((long)val, 16, add_plus, left_justify, width, *fmt == 'X', buf);
                fmt++;
                continue;
            } else {
                putchar('%');
        ...
        switch (*fmt) {
            ...
            case 's': {
                const unsigned char *val = (const unsigned char*)va_arg(args, const char*);
                if (val == NULL) val = (const unsigned char*)"(null)"; // NULL対策
...
int main(void) {
    ...
    printf("Float: %+-.3f, Int: %+5d, Long: %ld, HexLong: %lx, Ptr: %p, Exp: %e, HexFloat: %a, Uint: %u, UpperHex: %X, Str: %s\n",
