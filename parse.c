...
static Node *stmt(Token **rest, Token *tok) {
  ...
  if (equal(tok, "switch")) {
    ...
    char *brk = brk_label;
    brk_label = node->brk_label = new_jump_label();
  ...
  if (equal(tok, "for")) {
    ...
    brk_label = node->brk_label = new_jump_label();
    cont_label = node->cont_label = new_jump_label();
  ...
  if (tok->kind == TK_IDENT && equal(tok->next, ":")) {
    ...
    node->unique_label = new_jump_label();
...
static Node *to_assign(Node *binary) {
  ...
  if (binary->lhs->ty->is_atomic) {
    ...
    loop->brk_label = new_jump_label();
    loop->cont_label = new_jump_label();
...
//           | "<<=" | ">>="
//
// LONG costs differ by operator because of the code generated by opeq().
// Address generation is traded against save/restore costs.
//
static Node *assign(Token **rest, Token *tok) {
  ...
  if (equal(tok, "<<=")) {
    ...
    if (node->ty->kind == TY_LONG && is_simple_var(node)) {
      return new_binary(ND_ASSIGN,node,new_binary(ND_SHL,node,rhs,tok),tok);
    }
  ...
  if (equal(tok, ">>=")) {
    ...
    if (node->ty->kind == TY_LONG && is_simple_var(node)) {
      return new_binary(ND_ASSIGN,node,new_binary(ND_SHR,node,rhs,tok),tok);
    }
    ...
    while (!consume(&tok, tok, ";")) {
      ...
      if (consume(&tok, tok, ":")) {
        ...
        ||  (mem->bit_width == 0 && mem->name)) {
          error_tok(start, "invalid width for bit-field");
        }
      }
